  ...
  class RethinkOutput < BufferedOutput
    ...
    def configure(conf)
      ...
      if conf.has_key?('capped')
        raise ConfigError, "'capped_size' parameter is required on <store> of Rethink output" unless conf.has_key?('capped_size')
      ...
      # RethinkDB uses BSON's Date for time.
    ...
    def shutdown
      # Rethink::Connection checks alive or closed myself
    ...
    def operate(collection, records)
      ...
      rescue Rethink::OperationFailure => e
    ...
    def get_connection
      db = Rethink::RethinkClient.new(@host, @port, @connection_options).db(@database)
      authenticate(db)
    end
    ...
    def available_buffer_chunk_limit
      ...
      rescue Rethink::ConnectionFailure => e
    ...
    def mongod_version
      ...
      rescue Rethink::OperationFailure
        # fallback for buggy mongod version support
        version = authenticate(Rethink::RethinkClient.new(@host, @port, @connection_options).db('admin')).command('buildInfo' => 1)['version']
      end
